...
def handle_factor(account, train_year, train_month, latest_universe):
    ...
    for i in range(0, time_interval):  # 共7个月，每次循环后年月日要做相应的递增
        ...
        ********************************************************************
        ...
        BN_inv = []
        BC = []
        portfolios = []  # 存储18个组合，portfolios is a list of lists(list of tuples),[[(),()……,()],[]……,[]]
        port_revenue = []  # len is 18,是SH~BC 18个组合各个组合的平均收益率
        ...
        ********************************************************************
        ...
        for port_item in portfolios:  # port_item is SH~BC
            revenueSum = 0
            for item in port_item:
                closeOfFirst = item[3]
                for stock in lastDay_list:
                    if item[1] == stock[1]:
                        closeOfLast = stock[3]
                revenue = (closeOfLast - closeOfFirst) / closeOfFirst
                revenueSum += revenue
            item_revenue = revenueSum / len(port_item)
            port_revenue.append(item_revenue)
        ...
        ********************************************************************
        ...
        portfolios[SL,SM,SH,SW,SN_pro,SR,SC,SN_inv,SA,BL,BM,BH,BW,BN_pro,BR,BC,BN_inv,BA]
        ...
        SMB_BM_revenue = (portfolios[0]+portfolios[1]+portfolios[2])/3 - (portfolios[9]+portfolios[10]+portfolios[11])/3
        SMB_OP_revenue = (portfolios[5]+portfolios[4]+portfolios[3])/3 - (portfolios[14]+portfolios[13]+portfolios[12])/3
        SMB_inv_revenue=(portfolios[6]+portfolios[7]+portfolios[8])/3-(portfolios[15]+portfolios[16]+portfolios[17])/3
        SMB_revenue=(SMB_BM_revenue+SMB_OP_revenue+SMB_inv_revenue)/3
        HML_revenue = (portfolios[2]+portfolios[11])/2 - (portfolios[9]+portfolios[0])/2
        RMW_revenue=(portfolios[5]+portfolios[14])/2-(portfolios[3]+portfolios[12])/2
        CMA_revenue=(portfolios[6]+portfolios[15])/2-(portfolios[8]+portfolios[17])/2
